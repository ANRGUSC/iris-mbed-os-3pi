...
void _mqtt_thread()
{
    ...
    msg_for_main = main_thr_mailbox.alloc();
                            ...
                            case MQTT_PKT_TYPE:                                
                                ...
                                switch (mqtt_recv_data.data_type){
                                    ...
                                    case INM_DATA:
                                        ...
                                        mqtt_data_ptr = (char *)mqtt_recv_data.data;
                                        msg_for_main->type = INTER_THREAD;
                                        if (mqtt_data_ptr[0] == 'c') {
                                            PRINTF("Received current location coordinates\n");
                                            msg_for_main->type = CURRENT_LOCATION;
                                        }
                                        if (mqtt_data_ptr[0] == 't') {
                                            PRINTF("Received target location coordinates\n");
                                            msg_for_main->type = TARGET_LOCATION;
                                        }
                                        msg_for_main->content.ptr = mqtt_data_ptr;
                                        msg_for_main->sender_pid = osThreadGetId();
                                        main_thr_mailbox.put(msg_for_main);                                       
